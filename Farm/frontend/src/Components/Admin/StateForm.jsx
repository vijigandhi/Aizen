...
const StateForm = () => {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        const response = await fetch('http://localhost:8000/getCountries.php'); // Adjust endpoint as necessary
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('http://localhost:8000/addState.php', {
        method: 'POST',
        headers: {
